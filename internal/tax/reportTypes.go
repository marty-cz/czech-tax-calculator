...
type Report struct {
	...
	DividendRevenueWithNoTaxPayed *ValueAndFee
...
func (x *Report) String() string {
	return fmt.Sprintf("year: %d sellOpsCount:%d stock:(total:(revenue:(%v) fifoExpense:(%v)) timeTested:(revenue:(%v) fifoExpense:(%v))) dividend:(revenue:(%v) noPaidTaxRevenue(%v)) additional:revenue:(%v)",
		...
		x.DividendRevenue, x.DividendRevenueWithNoTaxPayed, x.AdditionalRevenue)
}
